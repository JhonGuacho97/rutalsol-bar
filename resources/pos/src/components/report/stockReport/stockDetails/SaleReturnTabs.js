...
const SaleReturnTabs = (props) => {
    ...
    useEffect(() => {
        if (isWarehouseValue === true) {
            downloadExcel(`get-product-sale-return-report-excel?product_id=${id}`, 'product_sale_return_report_url', () => setIsWarehouseValue(false));
        }
    }, [isWarehouseValue]);
    ...
    const itemsValue =
        ...
        stockDetailSaleReturn.map((sale) => ({
            ...
            date: dayjs(sale.attributes.date).format("YYYY-MM-DD"),
