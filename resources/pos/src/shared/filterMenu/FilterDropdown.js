...
const FilterDropdown = (props) => {
    const {
        ...
        fetchAllBaseUnits,
        base,
        isLoginStatus,        // ✅ nueva prop
        loginStatusOptions,   // ✅ nuevas opciones
        onLoginStatusChange,  // ✅ nuevo handler
        loginStatus,
    } = props;
    ...
    // ✅ Después - seguro aunque llegue undefined
    const loginStatusDefaultValue = loginStatusOptions
        ? getFormattedOptions(loginStatusOptions).map((option) => ({
            value: option.id,
            label: option.name,
        }))
        : [];
            ...
            <Dropdown.Menu className='px-7 py-5'>
                ...
                {isLoginStatus ?
                    <Dropdown.Header onClick={(e) => {
                        ...
                        <ReactSelect
                            multiLanguageOption={loginStatusOptions}
                            onChange={onLoginStatusChange}
                            name='login_status'
                            title={getFormattedMessage('login-logs.title')}
                            value={isReset ? loginStatusDefaultValue?.[0] : loginStatus}
                            defaultValue={loginStatusDefaultValue?.[0]}
                            placeholder={getFormattedMessage('login-logs.title')}
                        />
                    </Dropdown.Header>
                    : null}
                {isPaymentStatus ?
